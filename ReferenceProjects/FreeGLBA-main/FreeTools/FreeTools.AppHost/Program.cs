...
name: "--target",
description: "Target project to analyze (default: 'FreeGLBA')",
getDefaultValue: () => "FreeGLBA");
...
static class AppHostRunner
{
    ...
    public static void Run(bool skipCleanup, int keepBackups, string target)
    {
        ...
        // =============================================================================
        // Web App - Run FreeGLBA for HTTP testing
        // =============================================================================
        var webApp = builder.AddProject<Projects.FreeGLBA>("FreeGLBA-webapp")
            .WithEnvironment("ASPNETCORE_ENVIRONMENT", "Development");
        
        Console.WriteLine("  [WebApp] FreeGLBA - Development mode");
        ...
        var clientProjectRoot = Path.GetFullPath(Path.Combine(repoRoot, "FreeGLBA.Client"));
        ...
        // =============================================================================
        // FreeGLBA Test Client - Run API tests after poker confirms endpoints work
        // =============================================================================
        var testClient = builder.AddProject<Projects.FreeGLBA_TestClient>("glba-test-client")
            .WithEnvironment("FreeGLBA__Endpoint", webApp.GetEndpoint("https"))
            .WithEnvironment("START_DELAY_MS", (WebAppStartupDelayMs + HttpToolDelayMs).ToString())
            .WaitFor(webApp)
            .WaitForCompletion(poker);

        Console.WriteLine("  [TestClient] FreeGLBA.TestClient - API test suite");
