# 🌐 Site Report: https://help.em.wsu.edu/

> **Status:** ✅ 1/1 pages OK  
> **Folder:** `help-em-wsu-edu/`  

---

## 📋 Summary

```
Success Rate:  [██████████████████████████████] 100%
```

| Metric | Value |
|--------|-------|
| Pages Scanned | 1 |
| Pages Passed | ✅ 1 |
| Pages Failed | 0 |
| Total JS Errors | 0 |
| Total JS Warnings | 0 |
| Total Images | 1 (by URL) |
| Images Missing Alt | ⚠️ 1 |
| A11y Violations | ⚠️ 9 |
| 🔴 Critical | 1 |
| 🟠 Serious | 6 |
| 🟡 Moderate | 2 |
| 🔵 Minor | 0 |
| Total HTML | 18.1 KB |
| Total Screenshots | 19.5 KB |

## 🔒 SSL Certificate

| Field | Value |
|-------|-------|
| Subject | `CN=help.em.wsu.edu, O=Washington State University, S=Washington, C=US` |
| Issuer | `CN=InCommon RSA Server CA 2, O=Internet2, C=US` |
| Valid From | 2026-01-11 |
| Expires | 🟢 2027-01-12 (327 days) |
| Algorithm | sha256RSA |
| Key Size | 2048 bits |
| Thumbprint | `7A5042801D64B5CAA9661F8944CDCCC040841F19` |
| SANs | 1 domain(s) |

<details>
<summary><strong>Subject Alternative Names (1)</strong></summary>

| Domain | Type |
|--------|------|
| `help.em.wsu.edu` | 🏫 WSU |

</details>

## 📑 Pages

| Status | Page | HTTP | Title | 🔴 | 🟠 | 🟡 | 🔵 | A11y |
|:------:|------|:----:|-------|:--:|:--:|:--:|:--:|:----:|
| ✅ | [/](_root/report.md) | 200 | HelpDesk | 1 | 6 | 2 |  | ⚠️ 9 |

## 📸 Page Screenshots

Click any thumbnail to view the full page report.

<table>
<tr>
<td align="center" width="33%">
<a href="_root/report.md">
<img src="_root/01-page-loaded.jpg" width="250" alt="/" />
</a>
<br />✅ <code>/</code>
</td>
<td></td>
<td></td>
</tr>
</table>

## ♿ Accessibility Summary

| Metric | Value |
|--------|-------|
| Pages with violations | 1/1 |
| Total violations | 9 |
| 🔴 Critical | 1 |
| 🟠 Serious | 6 |
| 🟡 Moderate | 2 |
| 🔵 Minor | 0 |

### Top 4 Issues

| # | Rule | Sev | Pages | Instances |
|--:|------|:---:|:-----:|:---------:|
| 1 | [image-alt](../a11y-rules.md#image-alt) | 🔴 | 1/1 | 2 |
| 2 | [link-name](../a11y-rules.md#link-name) | 🟠 | 1/1 | 5 |
| 3 | [skip-link](../a11y-rules.md#skip-link) | 🟡 | 1/1 | 1 |
| 4 | [landmark-one-main](../a11y-rules.md#landmark-one-main) | 🟡 | 1/1 | 1 |

---

*Generated by AccessibilityScanner (FreeTools) v1.0*
